...
### Convention pause V1
Le secteur `Pause` doit exister comme vrai secteur visible dans les données de base.

Règle métier retenue :
- une entrée de pause a `sectorId = "pause"` ;
- une entrée de pause a `isPause = true` ;
- une entrée non pause a `isPause = false` et `sectorId !== "pause"`.
...
### Convention pause à prévoir dans les données de base
Un secteur par défaut doit être créé :
...
export const timeEntryRepository = {
  ...
  async listPauseEntriesByDate(date: string): Promise<TimeEntry[]> {
    return db.timeEntries.where("[date+isPause]").equals([date, true]).toArray();
  },
};
```
...
### Décision pause figée pour ce squelette
Le squelette doit être construit avec la convention suivante :
- un vrai secteur visible `Pause` ;
- des entrées de pause avec `sectorId = "pause"` ;
- des entrées de pause avec `isPause = true` ;
- les calculs métier travail / pause s’appuient prioritairement sur `isPause`.
